...
from datasets.DGR.base_loader import *
from datasets.DGR.transforms import *
from DGRutil.pointcloud import get_matching_indices, make_open3d_point_cloud, draw_registration_result
...
class KITTIPairDataset(PairDataset):
  AUGMENT = None
  DATA_FILES = {
      'train': './datasets/DGR/split/train_kitti.txt',
      'val': './datasets/DGR/split/val_kitti.txt',
      'test': './datasets/DGR/split/test_kitti.txt'
  }
  ...
  def __getitem__(self, idx):
    ...
    # use icp to obtain more accurate prediction
    ...
    if key not in kitti_icp_cache:
      if not os.path.exists(filename):
        ...
        # 将Kitti的真值（相机坐标系）转换到雷达坐标系下。虽然这句代码可读性不高
        ...
        # draw_registration_result(pcd0, pcd1, reg.transformation)
    ...
    if self.random_rotation:
      T0 = sample_random_trans(xyz0, self.randg, 45)
      T1 = sample_random_trans(xyz1, self.randg, 45)
...
# sample a pair at certain distance
class KITTINMPairDataset(KITTIPairDataset):
  ...
  def __init__(self,
    ...
    for dirname in subset_names:
      drive_id = int(dirname)
      print(root + '/sequences/%02d/velodyne/*.bin' % drive_id)
...
# sample a pair at certain distance
class SJTUNMPairDataset(KITTINMPairDataset):
  r"""
  Generate SJTU pairs within N meter distance
  """
  # 和kitti数据集不同，我们的真值就是雷达坐标系下的
  _velo2cam = np.eye(4)
  MIN_DIST = 5
  DATA_FILES = {
      'train': './datasets/DGR/split/train_sjtu.txt',
      'val': './datasets/DGR/split/val_sjtu.txt',
      'test': './datasets/DGR/split/test_sjtu.txt'
  }
