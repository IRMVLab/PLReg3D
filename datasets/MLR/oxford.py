...
class OxfordDataset(Dataset):
    ...
    Dataset wrapper for Oxford laser scans dataset from PointNetVLAD project.
    """
    def __init__(self, dataset_path, query_path, query_filename, transform=None, set_transform=None, max_elems=None, num_points=4096):
        ...
        assert os.path.exists(query_path), 'Cannot access query path: {}'.format(query_path)
        self.dataset_path = dataset_path
        self.query_path = query_path
        self.query_filepath = os.path.join(query_path, query_filename)
        assert os.path.exists(self.query_filepath), 'Cannot access query file: {}'.format(self.query_filepath)
        self.transform = transform
        self.set_transform = set_transform
        self.max_elems = max_elems
        self.n_points = num_points    # pointclouds in the dataset are downsampled to 4096 points
...
if __name__ == '__main__':
    ...
    query_path = '/media/sf_Datasets/PointNetVLAD'
    query_filename = 'test_queries_baseline.pickle'
    my_dataset = OxfordDataset(dataset_path, query_path, query_filename)
