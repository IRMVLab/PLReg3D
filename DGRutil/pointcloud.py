...
def find_matching_lines(cloud1_pcd, cloud2_pcd, correspondences, color):
  # connect the corresponding cloud points
  line_pcd = o3d.geometry.LineSet()
  line_pcd = line_pcd.create_from_point_cloud_correspondences(cloud1_pcd, cloud2_pcd, correspondences)
...
def pointcloud_to_boxes(pcd, voxel_size, color, box_scale=0.6):
  boxes = o3d.geometry.TriangleMesh()
  box_size = box_scale * voxel_size
  b = o3d.geometry.TriangleMesh.create_box(box_size, box_size, box_size)
  b.paint_uniform_color(color)

  if isinstance(pcd, o3d.geometry.PointCloud):
    pcd_points = np.array(pcd.points)
  for i, pcd_point in enumerate(pcd_points):
    one_box = copy.deepcopy(b)
    trans = np.identity(4)
    trans[:3, 3] = pcd_point
    one_box.transform(trans)
    boxes += one_box
