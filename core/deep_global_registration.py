...
from teaser_util.teaser_registration import teaser_feat
from misc.plot_util import plot_reg
from models import load_model

from core.registration import GlobalRegistration
from core.knn import find_knn_gpu

from DGRutil.timer import Timer
from DGRutil.pointcloud import make_open3d_point_cloud, pointcloud_to_boxes, pointcloud_to_spheres, find_matching_lines
...
class DeepGlobalRegistration:
  def __init__(self, config, device=torch.device('cuda')):
    ...
    if 'state_dict_inlier' in state.keys():
        ...
        print("load state_dict_inlier")
    else:
        print("can't find fcgf_model")
    ...
    if 'state_dict_inlier' in state.keys():
        ...
        print("load inlier_model")
    else:
        print("can't find inlier_model")
  ...
  def safeguard_registration(self, pcd0, pcd1, idx0, idx1, feats0, feats1,
    ...
    elif self.safeguard_method == 'teaser++':
        pcl0 = np.asarray(pcd0.points)
        pcl1 = np.asarray(pcd1.points)
        feats0 = feats0.detach().cpu().numpy()
        feats1 = feats1.detach().cpu().numpy()
        T = teaser_feat(pcl0, pcl1, feats0, feats1, self.voxel_size, VISUALIZE=False)
    else:
  ...
  def register(self, xyz0, xyz1, inlier_thr=0.00):
    ...
    # at least 200 pairs or 5% of all points are matched
