...
from DGRutil.timer import Timer, AverageMeter
from DGRutil.file import ensure_dir
...
class WeightedProcrustesTrainer:
  def __init__(self, config, data_loader, val_data_loader=None):
    ...
    # Model
    FeatModel = load_model(config.feat_model) #ResUNetBN2C
  ...
  def train(self):
    ...
    logging.info("-------Baseline random feature performance-------")
    if self.test_valid:
    ...
    for epoch in range(self.start_epoch, self.max_epoch + 1):
      ...
      if self.test_valid and epoch % self.val_epoch_freq == 0:
        ...
        if self.best_val < val_dict[self.best_val_metric]:
          ...
          self._save_checkpoint(epoch, 'best_val_checkpoint_simple')
        else:
          logging.info(
  ...
  def _train_epoch(self, epoch):
    ...
    for curr_iter in range(num_train_iter):
      ...
      for iter_idx in tqdm(range(iter_size)):
        data_timer.tic()
        ...
        # determin whether the point is inlier
        reg_soutput = self.inlier_model(reg_sinput)
        inlier_timer.toc()

        logits = reg_soutput.F
        # normalize to (0, 1) ,logit=0 represents weight=0.5
        weights = logits.sigmoid()

        # Truncate too low weights(takens as outliner)
        ...
        # Weighted Procrustes to calculate the pose
        ...
        # Select batches with at least 10 valid correspondences， ws为内点概率的求和
        valid_mask = ws > 10
        ...
        with torch.no_grad():
          ...
          pred = logits > 0  # todo thresh, considered true if pred>0
        ...
        # Backprop only if gradient is finite 避免梯度爆炸
      ...
      if curr_iter % self.config.stat_freq == 0:
        ...
        logging.info(' '.join([
            ...
            # ratio of correct match
            f"Correspondence acc: {correspondence_accuracy:.3e}",
            f", Precision: {precision:.4f}, Recall: {recall:.4f},",
            # f" F1: {f1:.4f}, TPR: {tpr:.4f}, TNR: {tnr:.4f}, BAcc: {balanced_accuracy:.4f}",
            f"trans err(m): {regist_rte_meter.avg:.3e}, rot err(deg): {regist_rre_meter.avg:.3e},",
            f"Succ rate: {regist_succ_meter.avg:3e}",
            f"Avg num valid every batch: {average_valid_meter.avg:3e}",
  ...
  def _valid_epoch(self):
    ...
    logging.info(' Valid: '.join([
        ...
        f"Loss: {loss_meter.avg}, Correspondence acc: {hit_ratio_meter.avg:.4f}, Precision: {precision}, Recall: {recall}, ",
        # f"F1: {f1}, TPR: {tpr}, TNR: {tnr}, BAcc: {balanced_accuracy}, ",
        f"trans err(m): {regist_rte_meter.avg:.3e}, rot err(deg): {regist_rre_meter.avg:.3e}, DGR Time: {dgr_timer.avg:.3e}",
        f"DGR Succ rate: {regist_succ_meter.avg:3e}",
    ]))
  ...
  def _load_weights(self, config):
    if config.resume is None and config.weights:
      ...
      if 'state_dict_inlier' in checkpoint:
        ...
        logging.warning("Inlier weight not found in '{}'".format(config.weights))
    ...
    if config.resume is not None:
      if osp.isfile(config.resume):
        ...
        if 'state_dict_inlier' in state:
          ...
          logging.warning("Inlier weights not found in '{}'".format(config.resume))
      else:
        logging.warning("Inlier weights does not exist at '{}'".format(config.resume))

  def _save_checkpoint(self, epoch, filename='checkpoint', simple=False):
    """
    Saving checkpoints
  ...
  def generate_inlier_input(self, xyz0, xyz1, iC0, iC1, iF0, iF1, len_batch, pos_pairs):
    ...
    # use the correct matching results
