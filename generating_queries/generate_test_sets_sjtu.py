...
def construct_query_and_database_sets(base_path, runs_folder, folders, pointcloud_fols, filename, output_name):
    ...
    for folder in folders:
        ...
        df_locations = pd.read_csv(os.path.join(base_path, runs_folder, all_folders[0], filename), sep=',')
    ...
    for folder in folders:
        ...
        test = {}
        df_locations = pd.read_csv(os.path.join(base_path, runs_folder, all_folders[0], filename), sep=',')
        df_locations['timestamp'] = runs_folder + all_folders[0] + pointcloud_fols + df_locations['timestamp'].astype(str) + '.bin'
