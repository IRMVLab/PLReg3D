...
from misc.log import setup_log, log_string, get_log_dir
...
def print_stats(stats, phase):
    if 'num_pairs' in stats:
        ...
        log_string(s.format(phase, stats['loss'], stats['avg_embedding_norm'], stats['num_pos'], stats['num_neg']))
...
def do_train(dataloaders, params: MinkLocParams, debug=False, visualize=False):
    ...
    log_string('Model name: {}'.format(model_name))
    ...
    if hasattr(model, 'print_info'):
        ...
        log_string('Number of model parameters: {}'.format(n_params))
    ...
    for epoch in (range(1, params.epochs + 1)):
        for phase in phases:
            ...
            total_train = len(dataset_iter)

            for i in tqdm.tqdm(range(total_train)):
                try:
                    batch, positives_mask, negatives_mask = dataset_iter.next()
                except ValueError:
                    continue
                ...
                if n_positives == 0 or n_negatives == 0:
                    ...
                    log_string('WARNING: Skipping batch without positive or negative examples')
                    continue
